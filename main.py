from lldm.models import Campaign as CampaignModel
...
session = get_session()
statement = sqlmodel.select(CampaignModel)
campaigns = session.exec(statement)

st.table(pd.DataFrame(campaigns.all()))
